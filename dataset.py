...
class Walk:
    ...
    def __str__(self):
        ...
        print(
            "\tData shape: (%d,%d), Start-end: (%d,%d), Label: %d"
            % (
                self.data.shape[0],
                self.data.shape[1],
                self.startRow,
...
class Condition:
    ...
    def _loadCSVs(self):
        ...
        # Load Bitalino data instead of Rokoko data
        ...
        # Use Bitalino data 
        self.rb1Data = self.bitalino1Data
        self.rb2Data = self.bitalino2Data

        # Use Rokoko data
        self.r_rb1Data = self.rokoko1Data
        self.r_rb2Data = self.rokoko2Data
        # self.rb1Data = self.rokoko1Data
        # self.rb2Data = self.rokoko2Data

        # concatenate the data rb1Data and r_rb1Data
        self.concat_1 = torch.cat((self.rb1Data, self.r_rb1Data), dim=1)
        self.concat_2 = torch.cat((self.rb2Data, self.r_rb2Data), dim=1)

        self.rb1Data = self.concat_1
        self.rb2Data = self.concat_2
        # # Apply feature selection
        # self.rb1Data = self.rb1Data[:, self.featuresIdx]
        # self.rb2Data = self.rb2Data[:, self.featuresIdx]


        # # Apply feature selection
        # self.r_rb1Data = self.r_rb1Data[:, self.featuresIdx]
        # self.r_rb2Data = self.r_rb2Data[:, self.featuresIdx]
    ...
    def _createCSVPaths(self):
        ...
        paths = {
            "rokoko1Path": r1d_path,
            "rokoko2Path": r2d_path,
            "bitalino1Path": b1d_path,
            "bitalino2Path": b2d_path,
            "labels1Path": r1l_path,
            "labels2Path": r2l_path,
        }
...
class Subject:
    ...
    def __init__(self, id, folderPath, featuresIdx, winLength):
        ...
        self.controlCondition = Condition(
            id, folderPath, "Control", featuresIdx, winLength
        )
...
class SubjectDataset(Dataset):
    ...
    def __init__(
        ...
        splitPcts={"train": 80, "val": 20, "test": 0},
        seed=42,
    ):
    ...
    def _getSubjectIds(self):
        ...
        folders = [
            ...
            if os.path.isdir(os.path.join(self.folderPath, f))
        ]
...
if __name__ == "__main__":
    ...
    # "Dataset "
    folderPath = "./MLSP_bit_dataset/train"
    # All of these subjects have missing data (NaNs)
    excludeIds = [
        "p011",
        "p069",
        "p012",
    ...
    # excludeIds = ["p021", "p288", "p028", "p051"]
    # These have no Rokoko data
    excludeIds.extend(["p021", "p288", "p028"])

    winLength = 100
    featuresIdx = list(range(0, 306+6))  # 306 for rokoko, 6 for bitalino
    splitPcts = {"train": 70, "val": 30}
    seed = 42

    datasetTr = SubjectDataset(
        folderPath,
        excludeIds,
        mode="train",
        winLength=winLength,
    ...
    torch.save(datasetTr, "train_dataset_class.pth")

    datasetVal = SubjectDataset(
        folderPath,
        excludeIds,
        mode="val",
        winLength=winLength,
    ...
    torch.save(datasetVal, "val_dataset_class.pth")

    seed = 42
    winLength = winLength
    splitPcts = {"train": 5, "val": 0}
    testDB = "./MLSP_bit_dataset/test"
    ...
    torch.save(test_dataset, "test_dataset_class.pth")
